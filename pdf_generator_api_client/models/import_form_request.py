...
IMPORTFORMREQUEST_ONE_OF_SCHEMAS = ["ImportFormBase64", "ImportFormUrl"]

class ImportFormRequest(BaseModel):
    """
    ImportFormRequest
    """
    # data type: ImportFormUrl
    oneof_schema_1_validator: Optional[ImportFormUrl] = None
    # data type: ImportFormBase64
    oneof_schema_2_validator: Optional[ImportFormBase64] = None
    actual_instance: Optional[Union[ImportFormBase64, ImportFormUrl]] = None
    one_of_schemas: Set[str] = { "ImportFormBase64", "ImportFormUrl" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ImportFormRequest.model_construct()
        error_messages = []
        match = 0
        # validate data type: ImportFormUrl
        if not isinstance(v, ImportFormUrl):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ImportFormUrl`")
        else:
            match += 1
        # validate data type: ImportFormBase64
        if not isinstance(v, ImportFormBase64):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ImportFormBase64`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ImportFormRequest with oneOf schemas: ImportFormBase64, ImportFormUrl. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ImportFormRequest with oneOf schemas: ImportFormBase64, ImportFormUrl. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ImportFormUrl
        try:
            instance.actual_instance = ImportFormUrl.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into ImportFormBase64
        try:
            instance.actual_instance = ImportFormBase64.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ImportFormRequest with oneOf schemas: ImportFormBase64, ImportFormUrl. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ImportFormRequest with oneOf schemas: ImportFormBase64, ImportFormUrl. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ImportFormBase64, ImportFormUrl]]:
