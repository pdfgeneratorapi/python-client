...
IMPORTTEMPLATEREQUEST_ONE_OF_SCHEMAS = ["ImportTemplateBase64", "ImportTemplateUrl"]

class ImportTemplateRequest(BaseModel):
    """
    ImportTemplateRequest
    """
    # data type: ImportTemplateUrl
    oneof_schema_1_validator: Optional[ImportTemplateUrl] = None
    # data type: ImportTemplateBase64
    oneof_schema_2_validator: Optional[ImportTemplateBase64] = None
    actual_instance: Optional[Union[ImportTemplateBase64, ImportTemplateUrl]] = None
    one_of_schemas: Set[str] = { "ImportTemplateBase64", "ImportTemplateUrl" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ImportTemplateRequest.model_construct()
        error_messages = []
        match = 0
        # validate data type: ImportTemplateUrl
        if not isinstance(v, ImportTemplateUrl):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ImportTemplateUrl`")
        else:
            match += 1
        # validate data type: ImportTemplateBase64
        if not isinstance(v, ImportTemplateBase64):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ImportTemplateBase64`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ImportTemplateRequest with oneOf schemas: ImportTemplateBase64, ImportTemplateUrl. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ImportTemplateRequest with oneOf schemas: ImportTemplateBase64, ImportTemplateUrl. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ImportTemplateUrl
        try:
            instance.actual_instance = ImportTemplateUrl.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into ImportTemplateBase64
        try:
            instance.actual_instance = ImportTemplateBase64.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ImportTemplateRequest with oneOf schemas: ImportTemplateBase64, ImportTemplateUrl. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ImportTemplateRequest with oneOf schemas: ImportTemplateBase64, ImportTemplateUrl. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ImportTemplateBase64, ImportTemplateUrl]]:
