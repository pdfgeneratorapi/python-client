...
class FormFieldsInner(BaseModel):
    """
    Form field definition
    """ # noqa: E501
    label: Optional[StrictStr] = Field(default=None, description="Field label displayed in the form")
    name: Optional[StrictStr] = Field(default=None, description="Data field name. For example \"name\" can be used as \"{name}\" in the document as placeholder.")
    type: Optional[StrictStr] = Field(default=None, description="Field type")
    required: Optional[StrictBool] = Field(default=None, description="Specifies if the field is required or not")
    __properties: ClassVar[List[str]] = ["label", "name", "type", "required"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['text', 'integer', 'number', 'date', 'signature']):
            raise ValueError("must be one of enum values ('text', 'integer', 'number', 'date', 'signature')")
        return value
