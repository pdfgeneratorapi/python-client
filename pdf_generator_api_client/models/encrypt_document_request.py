...
ENCRYPTDOCUMENTREQUEST_ONE_OF_SCHEMAS = ["EncryptAndDecryptBase64", "EncryptAndDecryptUrl"]

class EncryptDocumentRequest(BaseModel):
    """
    EncryptDocumentRequest
    """
    # data type: EncryptAndDecryptUrl
    oneof_schema_1_validator: Optional[EncryptAndDecryptUrl] = None
    # data type: EncryptAndDecryptBase64
    oneof_schema_2_validator: Optional[EncryptAndDecryptBase64] = None
    actual_instance: Optional[Union[EncryptAndDecryptBase64, EncryptAndDecryptUrl]] = None
    one_of_schemas: Set[str] = { "EncryptAndDecryptBase64", "EncryptAndDecryptUrl" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = EncryptDocumentRequest.model_construct()
        error_messages = []
        match = 0
        # validate data type: EncryptAndDecryptUrl
        if not isinstance(v, EncryptAndDecryptUrl):
            error_messages.append(f"Error! Input type `{type(v)}` is not `EncryptAndDecryptUrl`")
        else:
            match += 1
        # validate data type: EncryptAndDecryptBase64
        if not isinstance(v, EncryptAndDecryptBase64):
            error_messages.append(f"Error! Input type `{type(v)}` is not `EncryptAndDecryptBase64`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in EncryptDocumentRequest with oneOf schemas: EncryptAndDecryptBase64, EncryptAndDecryptUrl. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in EncryptDocumentRequest with oneOf schemas: EncryptAndDecryptBase64, EncryptAndDecryptUrl. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into EncryptAndDecryptUrl
        try:
            instance.actual_instance = EncryptAndDecryptUrl.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into EncryptAndDecryptBase64
        try:
            instance.actual_instance = EncryptAndDecryptBase64.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into EncryptDocumentRequest with oneOf schemas: EncryptAndDecryptBase64, EncryptAndDecryptUrl. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into EncryptDocumentRequest with oneOf schemas: EncryptAndDecryptBase64, EncryptAndDecryptUrl. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], EncryptAndDecryptBase64, EncryptAndDecryptUrl]]:
