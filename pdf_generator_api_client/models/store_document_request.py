...
class StoreDocumentRequest(BaseModel):
    ...
    prefill: Optional[PrefillParam] = None
    name: Optional[StrictStr] = Field(default='', description="Generated document name (optional)")
    output: Optional[StrictStr] = Field(default='url', description="Response format. `url` returns a public URL to the stored document; `viewer` returns a public URL to the PDF viewer.")
    __properties: ClassVar[List[str]] = ["file_base64", "file_url", "prefill", "name", "output"]
    ...
    def output_validate_enum(cls, value):
        ...
        if value not in set(['url', 'viewer']):
            raise ValueError("must be one of enum values ('url', 'viewer')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "prefill": PrefillParam.from_dict(obj["prefill"]) if obj.get("prefill") is not None else None,
            "name": obj.get("name") if obj.get("name") is not None else '',
            "output": obj.get("output") if obj.get("output") is not None else 'url'
        })
        return _obj
