...
class MakeAccessibleBase64(BaseModel):
    """
    MakeAccessibleBase64
    """ # noqa: E501
    file_base64: StrictStr = Field(description="PDF document in base64 encoded string format")
    options: Optional[List[AccessibilityOption]] = Field(default=None, description="Accessibility processing options")
    name: Optional[StrictStr] = Field(default=None, description="Name for the PDF file")
    output: Optional[StrictStr] = Field(default='base64', description="Returned document output format")
    metadata: Optional[MetadataParam] = None
    __properties: ClassVar[List[str]] = ["file_base64", "options", "name", "output", "metadata"]
    ...
    def output_validate_enum(cls, value):
        ...
        if value not in set(['base64', 'file', 'url', 'viewer']):
            raise ValueError("must be one of enum values ('base64', 'file', 'url', 'viewer')")
        return value
