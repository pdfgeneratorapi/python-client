...
FILLFORMFIELDSREQUEST_ONE_OF_SCHEMAS = ["FormFillBase64", "FormFillUrl"]

class FillFormFieldsRequest(BaseModel):
    """
    FillFormFieldsRequest
    """
    # data type: FormFillUrl
    oneof_schema_1_validator: Optional[FormFillUrl] = None
    # data type: FormFillBase64
    oneof_schema_2_validator: Optional[FormFillBase64] = None
    actual_instance: Optional[Union[FormFillBase64, FormFillUrl]] = None
    one_of_schemas: Set[str] = { "FormFillBase64", "FormFillUrl" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = FillFormFieldsRequest.model_construct()
        error_messages = []
        match = 0
        # validate data type: FormFillUrl
        if not isinstance(v, FormFillUrl):
            error_messages.append(f"Error! Input type `{type(v)}` is not `FormFillUrl`")
        else:
            match += 1
        # validate data type: FormFillBase64
        if not isinstance(v, FormFillBase64):
            error_messages.append(f"Error! Input type `{type(v)}` is not `FormFillBase64`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in FillFormFieldsRequest with oneOf schemas: FormFillBase64, FormFillUrl. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in FillFormFieldsRequest with oneOf schemas: FormFillBase64, FormFillUrl. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into FormFillUrl
        try:
            instance.actual_instance = FormFillUrl.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into FormFillBase64
        try:
            instance.actual_instance = FormFillBase64.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into FillFormFieldsRequest with oneOf schemas: FormFillBase64, FormFillUrl. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into FillFormFieldsRequest with oneOf schemas: FormFillBase64, FormFillUrl. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], FormFillBase64, FormFillUrl]]:
