...
FORMCONFIGURATIONNEWACTIONSINNER_ANY_OF_SCHEMAS = ["FormActionDownload", "FormActionSend", "FormActionStore"]

class FormConfigurationNewActionsInner(BaseModel):
    """
    FormConfigurationNewActionsInner
    """

    # data type: FormActionStore
    anyof_schema_1_validator: Optional[FormActionStore] = None
    # data type: FormActionDownload
    anyof_schema_2_validator: Optional[FormActionDownload] = None
    # data type: FormActionSend
    anyof_schema_3_validator: Optional[FormActionSend] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[FormActionDownload, FormActionSend, FormActionStore]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "FormActionDownload", "FormActionSend", "FormActionStore" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = FormConfigurationNewActionsInner.model_construct()
        error_messages = []
        # validate data type: FormActionStore
        if not isinstance(v, FormActionStore):
            error_messages.append(f"Error! Input type `{type(v)}` is not `FormActionStore`")
        else:
            return v

        # validate data type: FormActionDownload
        if not isinstance(v, FormActionDownload):
            error_messages.append(f"Error! Input type `{type(v)}` is not `FormActionDownload`")
        else:
            return v

        # validate data type: FormActionSend
        if not isinstance(v, FormActionSend):
            error_messages.append(f"Error! Input type `{type(v)}` is not `FormActionSend`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in FormConfigurationNewActionsInner with anyOf schemas: FormActionDownload, FormActionSend, FormActionStore. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[FormActionStore] = None
        try:
            instance.actual_instance = FormActionStore.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[FormActionDownload] = None
        try:
            instance.actual_instance = FormActionDownload.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_3_validator: Optional[FormActionSend] = None
        try:
            instance.actual_instance = FormActionSend.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into FormConfigurationNewActionsInner with anyOf schemas: FormActionDownload, FormActionSend, FormActionStore. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], FormActionDownload, FormActionSend, FormActionStore]]:
