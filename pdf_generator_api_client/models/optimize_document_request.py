...
OPTIMIZEDOCUMENTREQUEST_ONE_OF_SCHEMAS = ["OptimizeBase64", "OptimizeUrl"]

class OptimizeDocumentRequest(BaseModel):
    """
    OptimizeDocumentRequest
    """
    # data type: OptimizeUrl
    oneof_schema_1_validator: Optional[OptimizeUrl] = None
    # data type: OptimizeBase64
    oneof_schema_2_validator: Optional[OptimizeBase64] = None
    actual_instance: Optional[Union[OptimizeBase64, OptimizeUrl]] = None
    one_of_schemas: Set[str] = { "OptimizeBase64", "OptimizeUrl" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = OptimizeDocumentRequest.model_construct()
        error_messages = []
        match = 0
        # validate data type: OptimizeUrl
        if not isinstance(v, OptimizeUrl):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OptimizeUrl`")
        else:
            match += 1
        # validate data type: OptimizeBase64
        if not isinstance(v, OptimizeBase64):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OptimizeBase64`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in OptimizeDocumentRequest with oneOf schemas: OptimizeBase64, OptimizeUrl. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in OptimizeDocumentRequest with oneOf schemas: OptimizeBase64, OptimizeUrl. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into OptimizeUrl
        try:
            instance.actual_instance = OptimizeUrl.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into OptimizeBase64
        try:
            instance.actual_instance = OptimizeBase64.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into OptimizeDocumentRequest with oneOf schemas: OptimizeBase64, OptimizeUrl. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into OptimizeDocumentRequest with oneOf schemas: OptimizeBase64, OptimizeUrl. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], OptimizeBase64, OptimizeUrl]]:
