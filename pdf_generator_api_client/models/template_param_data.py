...
TEMPLATEPARAMDATA_ONE_OF_SCHEMAS = ["List[object]", "object"]

class TemplateParamData(BaseModel):
    """
    TemplateParamData
    """
    # data type: object
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = TemplateParamData.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in TemplateParamData with oneOf schemas: List[object], object. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in TemplateParamData with oneOf schemas: List[object], object. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into TemplateParamData with oneOf schemas: List[object], object. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into TemplateParamData with oneOf schemas: List[object], object. Details: " + ", ".join(error_messages))
        else:
            return instance
