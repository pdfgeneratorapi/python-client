...
OPENEDITORREQUESTDATA_ONE_OF_SCHEMAS = ["List[object]", "object"]

class OpenEditorRequestData(BaseModel):
    """
    Data used to generate the document. This can be an object or array of objects.
    """
    # data type: object
    oneof_schema_1_validator: Optional[Dict[str, Any]] = Field(default=None, description="JSON data used to replace data fields in the template")
    # data type: List[object]
    oneof_schema_2_validator: Optional[List[Dict[str, Any]]] = Field(default=None, description="JSON data used to replace data fields in the template")
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = OpenEditorRequestData.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in OpenEditorRequestData with oneOf schemas: List[object], object. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in OpenEditorRequestData with oneOf schemas: List[object], object. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into OpenEditorRequestData with oneOf schemas: List[object], object. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into OpenEditorRequestData with oneOf schemas: List[object], object. Details: " + ", ".join(error_messages))
        else:
            return instance
