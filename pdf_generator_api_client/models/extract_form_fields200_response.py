...
class ExtractFormFields200Response(BaseModel):
    """
    ExtractFormFields200Response
    """ # noqa: E501
    response: Optional[Dict[str, ExtractFormFields200ResponseResponseValue]] = Field(default=None, description="Form fields extracted from the PDF document")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each value in response (dict)
        _field_dict = {}
        if self.response:
            for _key_response in self.response:
                if self.response[_key_response]:
                    _field_dict[_key_response] = self.response[_key_response].to_dict()
            _dict['response'] = _field_dict
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "response": dict(
                (_k, ExtractFormFields200ResponseResponseValue.from_dict(_v))
                for _k, _v in obj["response"].items()
            )
