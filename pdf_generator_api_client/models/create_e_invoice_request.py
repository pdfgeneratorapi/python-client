...
class CreateEInvoiceRequest(BaseModel):
    ...
    """ # noqa: E501
    data: Dict[str, Any] = Field(description="JSON payload that represents the Peppol BIS Billing 3.0 UBL Invoice (https://docs.peppol.eu/poacc/billing/3.0/) Use the Get schema endpoint to see the detailed payload structure.")
    type: Optional[StrictStr] = Field(default='UBL', description="Formatting type.")
    output: Optional[StrictStr] = Field(default='base64', description="Response format. When the \"file\" option is used the API returns the file inline.")
    __properties: ClassVar[List[str]] = ["data", "type", "output"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['UBL', 'CII']):
            raise ValueError("must be one of enum values ('UBL', 'CII')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "data": obj.get("data"),
            "type": obj.get("type") if obj.get("type") is not None else 'UBL',
