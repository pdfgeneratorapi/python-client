...
from pdf_generator_api_client.models.watermark_file_url import WatermarkFileUrl
from pydantic import StrictStr, Field
from typing import Union, List, Set, Optional, Dict
from typing_extensions import Literal, Self

ADDWATERMARKREQUEST_ONE_OF_SCHEMAS = ["WatermarkBase64", "WatermarkFileUrl"]

class AddWatermarkRequest(BaseModel):
    """
    AddWatermarkRequest
    """
    # data type: WatermarkFileUrl
    oneof_schema_1_validator: Optional[WatermarkFileUrl] = None
    # data type: WatermarkBase64
    oneof_schema_2_validator: Optional[WatermarkBase64] = None
    actual_instance: Optional[Union[WatermarkBase64, WatermarkFileUrl]] = None
    one_of_schemas: Set[str] = { "WatermarkBase64", "WatermarkFileUrl" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = AddWatermarkRequest.model_construct()
        error_messages = []
        match = 0
        # validate data type: WatermarkFileUrl
        if not isinstance(v, WatermarkFileUrl):
            error_messages.append(f"Error! Input type `{type(v)}` is not `WatermarkFileUrl`")
        else:
            match += 1
        # validate data type: WatermarkBase64
        if not isinstance(v, WatermarkBase64):
            error_messages.append(f"Error! Input type `{type(v)}` is not `WatermarkBase64`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in AddWatermarkRequest with oneOf schemas: WatermarkBase64, WatermarkFileUrl. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in AddWatermarkRequest with oneOf schemas: WatermarkBase64, WatermarkFileUrl. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into WatermarkFileUrl
        try:
            instance.actual_instance = WatermarkFileUrl.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into WatermarkBase64
        try:
            instance.actual_instance = WatermarkBase64.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into AddWatermarkRequest with oneOf schemas: WatermarkBase64, WatermarkFileUrl. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AddWatermarkRequest with oneOf schemas: WatermarkBase64, WatermarkFileUrl. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], WatermarkBase64, WatermarkFileUrl]]:
