...
class GenerateDocumentAsynchronousRequest(BaseModel):
    """
    GenerateDocumentAsynchronousRequest
    """ # noqa: E501
    template: Optional[TemplateParam] = None
    callback: Optional[CallbackParam] = None
    format: Optional[FormatParam] = FormatParam.PDF
    output: Optional[AsyncOutputParam] = AsyncOutputParam.BASE64
    name: Optional[StrictStr] = Field(default='', description="Generated document name (optional)")
    testing: Optional[StrictBool] = Field(default=False, description="When set to true the generation is not counted as merge (monthly usage), but a large PREVIEW stamp is added.")
    make_accessible: Optional[StrictBool] = Field(default=False, description="Enables semantic document tagging. When enabled, a separate Make Accessible action is executed, which consumes additional credits.")
    metadata: Optional[MetadataParam] = None
    __properties: ClassVar[List[str]] = ["template", "callback", "format", "output", "name", "testing", "make_accessible", "metadata"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "template": TemplateParam.from_dict(obj["template"]) if obj.get("template") is not None else None,
            "callback": CallbackParam.from_dict(obj["callback"]) if obj.get("callback") is not None else None,
            "format": obj.get("format") if obj.get("format") is not None else FormatParam.PDF,
            "output": obj.get("output") if obj.get("output") is not None else AsyncOutputParam.BASE64,
            "name": obj.get("name") if obj.get("name") is not None else '',
            "testing": obj.get("testing") if obj.get("testing") is not None else False,
            "make_accessible": obj.get("make_accessible") if obj.get("make_accessible") is not None else False,
            "metadata": MetadataParam.from_dict(obj["metadata"]) if obj.get("metadata") is not None else None
        })
        return _obj
