...
class EncryptAndDecryptBase64(BaseModel):
    """
    EncryptAndDecryptBase64
    """ # noqa: E501
    file_base64: StrictStr = Field(description="PDF document in base64 encoded string format")
    owner_password: StrictStr = Field(description="An owner password to open the encrypted document")
    user_password: Optional[StrictStr] = Field(default=None, description="An user password to open the encrypted document")
    ...
    __properties: ClassVar[List[str]] = ["file_base64", "owner_password", "user_password", "name", "output"]
    ...
    def output_validate_enum(cls, value):
        ...
        if value not in set(['file', 'base64', 'url']):
            raise ValueError("must be one of enum values ('file', 'base64', 'url')")
        return value
