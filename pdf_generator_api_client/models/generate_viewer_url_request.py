...
class GenerateViewerUrlRequest(BaseModel):
    """
    GenerateViewerUrlRequest
    """ # noqa: E501
    output: Optional[StrictStr] = Field(default='url', description="Response format. `url` returns a public URL to the stored document; `viewer` returns a public URL to the PDF viewer (with encrypted prefill when provided).")
    prefill: Optional[PrefillParam] = None
    __properties: ClassVar[List[str]] = ["output", "prefill"]
