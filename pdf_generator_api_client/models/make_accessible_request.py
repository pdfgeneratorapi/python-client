...
MAKEACCESSIBLEREQUEST_ONE_OF_SCHEMAS = ["MakeAccessibleBase64", "MakeAccessibleUrl"]

class MakeAccessibleRequest(BaseModel):
    """
    MakeAccessibleRequest
    """
    # data type: MakeAccessibleUrl
    oneof_schema_1_validator: Optional[MakeAccessibleUrl] = None
    # data type: MakeAccessibleBase64
    oneof_schema_2_validator: Optional[MakeAccessibleBase64] = None
    actual_instance: Optional[Union[MakeAccessibleBase64, MakeAccessibleUrl]] = None
    one_of_schemas: Set[str] = { "MakeAccessibleBase64", "MakeAccessibleUrl" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = MakeAccessibleRequest.model_construct()
        error_messages = []
        match = 0
        # validate data type: MakeAccessibleUrl
        if not isinstance(v, MakeAccessibleUrl):
            error_messages.append(f"Error! Input type `{type(v)}` is not `MakeAccessibleUrl`")
        else:
            match += 1
        # validate data type: MakeAccessibleBase64
        if not isinstance(v, MakeAccessibleBase64):
            error_messages.append(f"Error! Input type `{type(v)}` is not `MakeAccessibleBase64`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in MakeAccessibleRequest with oneOf schemas: MakeAccessibleBase64, MakeAccessibleUrl. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in MakeAccessibleRequest with oneOf schemas: MakeAccessibleBase64, MakeAccessibleUrl. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into MakeAccessibleUrl
        try:
            instance.actual_instance = MakeAccessibleUrl.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into MakeAccessibleBase64
        try:
            instance.actual_instance = MakeAccessibleBase64.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into MakeAccessibleRequest with oneOf schemas: MakeAccessibleBase64, MakeAccessibleUrl. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into MakeAccessibleRequest with oneOf schemas: MakeAccessibleBase64, MakeAccessibleUrl. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], MakeAccessibleBase64, MakeAccessibleUrl]]:
